...
#include "horizontalelbow.h"
#include "paintHelper.h"

#include <QGuiApplication>
#include <QToolTip>

QString HorizontalElbow::SHAPE_ID = "HORIZONTAL_ELBOW_2024";
HorizontalElbow::HorizontalElbow(QGraphicsItem *parent)
    : AbstractLineItem(parent)
{
    auto attr = attribute();
    LineEndType start(LineEndType::Start, "");
    start.setFixRotation(0);
    attr->setStart(start);
    LineEndType end(LineEndType::End, "arcArrow");
    end.setFixRotation(0);
    attr->setEnd(end);
}

HorizontalElbow::HorizontalElbow(const QString &xml, QGraphicsItem *parent)
    : AbstractLineItem(parent)
{
    parseXML(xml);
}

QString HorizontalElbow::classId() const
{
    return SHAPE_ID;
}

void HorizontalElbow::onPointMoveStart(const QString &id)
{
    AbstractLineItem::onPointMoveStart(id);
    if (initStatge < 3) {
        // 未初始化完成，不启用
        return;
    }
...
void HorizontalElbow::onPointMoveEnd(const QString &id)
{
    AbstractLineItem::onPointMoveEnd(id);
    if (initStatge < 3) {
        return;
    }
    addFlag = false;
...
QPainterPath HorizontalElbow::linePath(QList<QPointF> points) const
{
    QPainterPath path;
    auto attr = attribute();
...
void HorizontalElbow::createPredefinedItem()
{
    auto attr = attribute();
...
void HorizontalElbow::afterCreateSelector(LineSelector *s)
{
...
void HorizontalElbow::afterInit()
{
}

void HorizontalElbow::afterPointMoved(const QString &id)
{
    auto attr = attribute();
    ...
    if (initStatge == 3 && id.compare(lastPt.getId()) == 0 &&
        !addFlag /*&& qAbs(lastPt.getPt().y() - lastPoint.y()) > 5*/
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (i == 0) {
            ...
            if (initStatge < 3) {
                ...
                if (count > 3) {
                    attr->updatePoint(list[i-2].id, {(list[i-1].pt.x() + list[i-3].pt.x())/2, list[i-2].pt.y()});
                }
            } else {
                attr->updatePoint(list[i-1].id, {list[i-1].pt.x(), (list[i].pt.y() + list[i-2].pt.y())/2});
            }
...
void HorizontalElbow::afterPointMoveEnd(const QString &id)
{
    Q_UNUSED(id)
    if (addFlag && initStatge == DRAWING) {
        addFlag = false;
        initStatge = FINISHED;
    }
}

void HorizontalElbow::beforeAddPoint(const QString &id, const QPointF &pos)
{
    auto attr = attribute();
    if (attr->getPointCount()%2 == 0){
        return;
    }
    // 添加中间点
    auto list = attr->getPointList();
...
void HorizontalElbow::beforeDelPoint(const QString &id)
{
    auto attr = attribute();
    auto list = attr->getPointList();
    // 删除中间点
...
void HorizontalElbow::updateEndRotaion()
{
    auto attr = attribute();
    auto list = attr->getPointList();
    ...
    if (!start.isEmpty()) {
        ...
        offset = list[0].pt.x() - list[1].pt.x();
        if (offset > minOffset) {
            start.setRotation(180);
        } else if (offset < -minOffset){
            start.setRotation(0);
        } else {
            if (list[0].pt.y() > list[1].pt.y()){
                start.setRotation(-90);
            } else {
                start.setRotation(90);
            }
            attr->updatePoint(list[0].id, {list[1].pt.x(), list[0].pt.y()});
        }
        attr->setStart(start);
    }
    ...
    if (!end.isEmpty()) {
        ...
        auto count = list.count();
        offset = list[count - 1].pt.x() - list[count - 2].pt.x();
        if (offset > minOffset) {
            end.setRotation(0);
        } else if (offset < -minOffset){
            end.setRotation(180);
        } else {
            if (list[count - 1].pt.y() < list[count - 2].pt.y()){
                end.setRotation(-90);
            } else {
                end.setRotation(90);
            }
            attr->updatePoint(list[count - 1].id, {list[count - 2].pt.x(), list[count - 1].pt.y()});
        }
        attr->setEnd(end);
    }
}

void HorizontalElbow::autoAddPoint()
{
    addFlag = true;
    ...
    auto count = list.count();
    // 添加2个点的位置
    auto x = (list[count-2].getPt().x() + list[count-1].getPt().x())/2;
    auto y = lastPoint.y();
...
void HorizontalElbow::autoMergePoit(const QString &movedId)
{
...
qreal HorizontalElbow::calcEndRotation(const QList<QPointF> &points, const LineEndType &type) const
{
    ...
    if (type.getCategory() == LineEndType::End) {
        ...
        if (points[count-1].x() > points[count-2].x()) {
            return 0;
        } else if (points[count-1].x() == points[count-2].x()){
